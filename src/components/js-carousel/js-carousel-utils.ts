...
export const checkSpecKeys = (spec, keysArray) => {
  ...
  return keysArray.reduce((value, key) => {
    if (value && spec.hasOwnProperty(key))
      return true
    else {
    ...
    ? null
    : console.error("Keys Missing:", spec, missingKeys);
};
...
export const getTrackCSS = spec => {
  ...
  let style: { opacity?: number; transition?: string; WebkitTransition?: string, WebkitTransform?: string, transform?: string, msTransform?: string, width?: number, height?: number, marginLeft?: string, marginTop?: string } = {
    opacity: 1,
    transition: "",
    WebkitTransition: ""
  };
